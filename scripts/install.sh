...
# Resolve release tag. Accept JOHTO_VERSION as bare version (0.1.1), v-prefixed (v0.1.1),
# or full tag (johto/v0.1.1); fall back to GitHub's latest-release API.
RAW_TAG="${JOHTO_VERSION:-$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" | grep '"tag_name"' | head -n1 | cut -d'"' -f4)}"
[ -z "$RAW_TAG" ] && { echo "Failed to resolve release tag" >&2; exit 1; }

VERSION="${RAW_TAG#johto/v}"
VERSION="${VERSION#v}"
TAG="johto/v$VERSION"

URL="https://github.com/$REPO/releases/download/$TAG/johto-$VERSION-$SUFFIX.tar.gz"
...
echo
echo "johto v$VERSION installed to $INSTALL_DIR"
